    ...
    <section className="">
      <div className="rounded-xl relative p-2 sm:p-3 md:p-4 flex flex-col justify-between w-full bg-white group transition-all border border-primary/30 duration-200 hover:shadow-md hover:border-primary">
        <div className="">
          ...
          {/* Product Image */}
          <div className="flex justify-center items-center h-20 sm:h-28 md:h-32 lg:h-36 object-fill w-full bg-gray-50 rounded-lg overflow-hidden">
          ...
          <div className="mt-2 sm:mt-3 flex flex-col flex-1">
            ...
            <Link
              ...
              className="text-[12px] sm:text-[14px] md:text-[15px] font-semibold text-gray-800 leading-tight mt-1 mb-1 sm:mb-2 line-clamp-2"
            >
              {product.name}
            </Link>
                ...
                <div className="flex gap-1  sm:gap-1.5 flex-nowrap overflow-x-auto pb-1" style={{ scrollbarWidth: 'thin', scrollbarColor: '#d1d5db transparent' }}>
                  {product.variants.map((v, idx) => (
                    <button
                      key={idx}
                      type="button"
                      className={`px-2 sm:px-3 py-1 rounded-lg cursor-pointer border text-[10px] sm:text-xs font-medium transition-colors whitespace-nowrap min-w-fit ${
            ...
            <div className="flex items-center justify-between mt-auto pt-1 px-1 flex-nowrap gap-2">
              ...
              {cartItem ? (
                <div className="flex items-center border border-primary/30 rounded-md overflow-hidden flex-shrink-0">
                  <button
                    onClick={handleDecrease}
                    className="p-1.5 sm:p-2 bg-gray-200 cursor-pointer hover:bg-gray-300 transition-colors duration-200"
                    aria-label={t("decrease_quantity")}
                  >
                    <Minus className="w-3 h-3 sm:w-4 sm:h-4 text-gray-700" />
                  </button>
                  <span className="px-2 sm:px-3 text-gray-800 text-sm">
                    {cartItem.quantity}
                  </span>
                  <button
                    onClick={handleIncrease}
                    className="p-1.5 sm:p-2 cursor-pointer bg-primary hover:bg-primary/80 text-white transition-colors duration-200"
                ...
                <button
                  className="flex cursor-pointer items-center gap-1 flex-nowrap border-primary border-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-[10px] sm:text-sm font-semibold hover:border-primary/80 transition-colors flex-shrink-0"
